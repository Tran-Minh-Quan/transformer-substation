...
def crop(dir_images, dir_bboxes, dir_folder):
    list_dir = listdir(dir_images)
    num_im = len(list_dir)
    for idx, filename in enumerate(list_dir):
        dir_im = dir_images + "/" + filename
        im = cv2.imread(dir_im)
        h, w = im.shape[:2]

        dir_bbox = dir_bboxes + "/" + splitext(filename)[0] + '.txt'
        if not exists(dir_bboxes):
            raise Exception(filename+' has not been labeled')
        bbox = [float(i) for i in open(dir_bbox, 'r').read().splitlines()[0].split(' ')[1:]]
        top_left = (round((bbox[0] - bbox[2] / 2) * w), round((bbox[1] - bbox[3] / 2) * h))
        bottom_right = (round((bbox[0] + bbox[2] / 2) * w), round((bbox[1] + bbox[3] / 2) * h))

        crop = im[top_left[1]:bottom_right[1], top_left[0]:bottom_right[0], :]

        dir_save = dir_folder + "/" + splitext(filename)[0] + '_crop' + '.png'
        cv2.imwrite(dir_save, crop)
        print(f'cropped {idx+1}/{num_im} image')
