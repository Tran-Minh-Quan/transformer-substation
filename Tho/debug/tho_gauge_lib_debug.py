...
class gauge_hand:
    ...
    def find_pointing(edges, img, quantity, thresh_low, min_length, max_gap, step, min_angle, max_angle):
        ...
        inc_thresh = True
        while inc_thresh:
            lines = cv2.HoughLinesP(edges, 0.5, np.pi/180, thresh_low,
        ...
        line_1_min = np.array([[lines[0, 0, 0], lines[0, 0, 1]], [lines[0, 0, 2], lines[0, 0, 3]]])
        line_2_min = np.array([[lines[1, 0, 0], lines[1, 0, 1]], [lines[1, 0, 2], lines[1, 0, 3]]])
        ...
        for i in range(lines.shape[0]):
            for j in range(i+1, lines.shape[0]):
                ...
                if seg_1.intersects(seg_2):
                    ...
                    cross_response = 2*np.sqrt(tip_dist_sqr) + np.sqrt(float_dist_sqr) - 0.3*(len_1 + len_2)
                    if cross_response_min > cross_response:
                        angle = gauge_hand.calc_angle_seg(lines[i, 0], lines[j, 0])
                        if ((min_angle_rad < angle < max_angle_rad) or
                                (min_angle_rad < abs(angle-np.pi) < max_angle_rad) or
                                (min_angle_rad < 2*np.pi - angle < max_angle_rad)):
        ...
        all_line = img.copy()
        for i in range(lines.shape[0]):
            cv2.line(all_line, tuple(np.array([lines[i, 0, 0], lines[i, 0, 1]])),
                     tuple(np.array([lines[i, 0, 2], lines[i, 0, 3]])), (0, 0, 255), 1)
        return gauge_hands, avg_angle, img_out, all_line


def get_temp_meter(img, resize_ratio, bounding_box):
    bounding_img = img[bounding_box[1]:bounding_box[3], bounding_box[0]:bounding_box[2], :]
    if resize_ratio == 1:
        img_resized = bounding_img.copy()
    else:
        img_resized = cv2.resize(bounding_img,  # Resize image for faster processing time
                                 (np.uint(bounding_img.shape[1]*resize_ratio),
                                  np.uint(bounding_img.shape[0]*resize_ratio)))
    # Red pointer processing
    # --------------------------------------------------- #
    img_1 = cv2.GaussianBlur(img_resized, (7, 7), 0)
    img_hsv = cv2.cvtColor(img_1, cv2.COLOR_BGR2HSV)  # Convert image to HSV
    ...
    img_masked = cv2.bitwise_and(img_1, img_1, mask=mask)  # Bitwise mask with img
    edges_1 = cv2.Canny(img_masked, 60, 180)  # Find edges in img
    # Find gauge hand in img
    _, gauge_angle, img_show_1, all_line_1 = gauge_hand.find_pointing(edges_1, img_resized, quantity=20,
                                                                      thresh_low=20, min_length=25, max_gap=30,
                                                                      step=5, min_angle=2, max_angle=30)
    # Interpolate to find value of meter
    temp_interp1d = interp1d(temp_meter.angle_data, temp_meter.value_data)
    red_read_val = temp_interp1d(gauge_angle)
    # White pointer processing
    # --------------------------------------------------#
    img_2 = cv2.GaussianBlur(img_resized, (3, 3), 0)
    img_hls = cv2.cvtColor(img_2, cv2.COLOR_BGR2HLS)
    low_thresh = np.array([15, 100, 28])
    high_thresh = np.array([36, 255, 255])
    mask = cv2.inRange(img_hls, low_thresh, high_thresh)
    img_threshold = cv2.bitwise_and(img_2, img_2, mask=mask)
    kernel = np.ones((5, 5), np.uint8)
    img_threshold = cv2.morphologyEx(img_threshold, cv2.MORPH_OPEN, kernel)
    edges_2 = cv2.Canny(img_threshold, 60, 180)
    _, gauge_angle, img_show_2, all_line_2 = gauge_hand.find_pointing(edges_2, img_resized, quantity=20,
                                                                      thresh_low=20, min_length=15, max_gap=20,
                                                                      step=5, min_angle=2, max_angle=30)
    temp_interp1d = interp1d(temp_meter.angle_data, temp_meter.value_data)
    white_read_val = temp_interp1d(gauge_angle)
    return red_read_val, white_read_val, img_show_1, img_show_2, edges_1, edges_2, all_line_1, all_line_2


# Temperature meter data
class temp_meter:
    angle_data = np.array([0, 0.8135597303853407, 5.504780036787471, 99])
    value_data = np.array([0, 0, 150, 150])


# Pressure meter data
class press_meter:
    angle_data = np.array([0, 0.7853981633974483, 5.497787143782138, 99])
    value_data = np.array([0, -1, 10, 10])
